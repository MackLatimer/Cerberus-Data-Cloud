...
def test_upload_voters(client, session, auth_token):
    """
    ...
    # Add auth header for the protected endpoint
    headers = {
        'Authorization': f'Bearer {auth_token}'
    }

    # Make a POST request to the upload endpoint
    response = client.post('/api/v1/voters/upload', data=data, content_type='multipart/form-data', headers=headers)

    # Assert that the request was successful
    assert response.status_code == 200
    assert response.json['message'] == 'File processed successfully'

    # Assert that the voters were actually added to the database.
