...
import csv
import io
from . import campaigns_api_bp, public_api_bp
...
@campaigns_api_bp.route('/voters', methods=['POST'])
...
@campaigns_api_bp.route('/voters', methods=['GET'])
# @auth_required
...
@campaigns_api_bp.route('/voters/<int:voter_id>', methods=['DELETE'])
# @auth_required
...
# Note: Authentication (@auth_required) is critical for the campaigns_api_bp endpoints
# and needs to be implemented. The public_api_bp /signups endpoint is intentionally public.

@campaigns_api_bp.route('/voters/upload', methods=['POST'])
def upload_voters():
