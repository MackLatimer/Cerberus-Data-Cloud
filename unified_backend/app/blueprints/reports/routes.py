...
@reports_bp.route('/subscribe', methods=['POST'])
def subscribe():
...
@reports_bp.route('/webhook/order_canceled', methods=['POST'])
def order_canceled():
    try:
...
@reports_bp.route('/webhook/order_renewed', methods=['POST'])
def order_renewed():
    try:
        # Outer try for request parsing and initial validation
        data = request.get_json()
...
@reports_bp.route('/municipalities', methods=['GET'])
...
@reports_bp.route('/headings', methods=['GET'])
def get_headings():
    conn = None
    cur = None
    try:
...
@reports_bp.route('/test_db', methods=['GET'])
def test_db():
    conn = None
...
@reports_bp.route('/debug_db_url', methods=['GET'])
def debug_db_url():
    env_vars_status = {
